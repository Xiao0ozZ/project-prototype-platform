<template>
  <div
    class="packages-page platform-page min-h-screen text-slate-900 font-sans antialiased selection:bg-blue-500 selection:text-white relative overflow-hidden"
  >
    <!-- 苹果光流背景 Gradient Orbs -->
    <div class="fixed inset-0 pointer-events-none z-0">
      <div
        class="absolute -top-40 right-0 w-[600px] h-[600px] rounded-full bg-blue-300/25 blur-[120px]"
      ></div>
      <div
        class="absolute bottom-0 -left-20 w-[600px] h-[600px] rounded-full bg-purple-300/25 blur-[120px]"
      ></div>
    </div>
    ...
    <main class="mx-auto max-w-6xl px-6 py-6 space-y-5 relative z-10">
      <!-- 1. 精简紧凑型 Header 顶栏 -->
      <header
        class="rounded-3xl bg-white/80 backdrop-blur-2xl border border-white/90 px-7 py-5 shadow-xl shadow-slate-200/50 space-y-3"
      >
        <!-- 顶行：返回首页 + 操作按钮群 -->
        ...
        <div class="flex items-center gap-3 pt-1">
          <div
            class="w-10 h-10 rounded-xl bg-gradient-to-tr from-blue-500 to-indigo-600 text-white shadow-md shadow-blue-500/20 flex items-center justify-center text-lg font-bold shrink-0"
          >
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              <span
                class="px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold bg-blue-50 text-blue-600 border border-blue-100 uppercase tracking-wider"
              >
                PROJECT LIBRARY
              </span>
            </div>
      ...
      <section
        class="rounded-3xl bg-white/80 backdrop-blur-2xl border border-white/90 px-7 py-5 shadow-xl shadow-slate-200/50 flex flex-col md:flex-row items-stretch justify-between gap-6"
        aria-label="项目包概览"
      >
        <div class="space-y-1 md:w-1/3 flex flex-col justify-center">
          <span class="text-[10px] font-mono font-bold text-blue-600 uppercase tracking-widest"
            >LOCAL PROJECT PACKAGES</span
          >
        ...
        <div
          class="grid grid-cols-1 sm:grid-cols-3 gap-3.5 md:border-l md:border-slate-200/80 md:pl-6 flex-1"
        >
          <div
            class="bg-white/60 backdrop-blur-xl rounded-2xl p-4 border border-white/80 shadow-2xs flex flex-col justify-center space-y-0.5"
          >
          ...
          <div
            class="bg-white/60 backdrop-blur-xl rounded-2xl p-4 border border-white/80 shadow-2xs flex flex-col justify-center space-y-0.5"
            :class="{ 'bg-amber-50/70 border-amber-200/80': invalidProjects.length }"
          >
            <div class="flex items-center justify-between">
              ...
              <span
                ...
                :class="
                  invalidProjects.length ? 'bg-amber-500 shadow-sm shadow-amber-500/50' : 'bg-slate-300'
                "
              ></span>
            </div>
            <span
              class="text-2xl font-extrabold tracking-tight"
              :class="invalidProjects.length ? 'text-amber-600' : 'text-slate-900'"
              >{{ invalidProjects.length }}</span
            >
            <span class="text-[10px] text-slate-400 font-medium">{{
              invalidProjects.length ? '需要处理后才能使用' : '当前没有异常'
            }}</span>
          </div>
          ...
          >
            ...
            <span class="text-xl font-extrabold text-blue-600 tracking-tight">{{
              canManageProjects ? '可管理' : '只读'
            }}</span>
            <span class="text-[10px] text-slate-400 font-medium">{{
              canManageProjects ? '可编辑项目包配置' : '仅查看生产构建内容'
            }}</span>
          </div>
        </div>
      </section>
      ...
      <section
        class="rounded-3xl bg-white/80 backdrop-blur-2xl border border-white/90 px-7 py-6 shadow-xl shadow-slate-200/50 space-y-4"
      >
        <div class="flex items-center justify-between">
          ...
          <span
            class="px-3 py-0.5 rounded-full text-xs font-bold bg-blue-50 text-blue-600 border border-blue-100 shadow-2xs"
          >
            {{ projects.length }} 个项目
          </span>
        </div>
        ...
        <div class="rounded-2xl border border-slate-200/70 overflow-hidden shadow-2xs">
          <el-table :data="projects" border empty-text="尚未发现有效项目包" class="apple-glass-table">
            ...
            <el-table-column label="状态" width="100">
              <template #default>
                <span
                  class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-50 text-emerald-600 border border-emerald-100 inline-flex items-center gap-1"
                >
      ...
      <!-- 4. 无效项目包 Warning (如果有) -->
      <section
        v-if="invalidProjects.length"
        class="rounded-3xl bg-amber-50/80 backdrop-blur-2xl border border-amber-200/80 px-7 py-5 shadow-xl shadow-amber-950/5 space-y-3"
      >
        <div class="flex items-center justify-between">
          <h2 class="text-base font-extrabold text-amber-900">无效项目包</h2>
          <span
            class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-amber-100 text-amber-800 border border-amber-200"
          >
